        ...
        <div class="flex flex-col gap-6 pt-8 lg:flex-row">
            <div class="px-6 lg:w-9/12">
                ...
                <div class="post">
                    ...
                    <figure class="mb-0 animated"><a href="{{route('Realisation.one',[$new->slug])}}"
                            title="{{$new->title}}"><img src="{{$new->getFirstMediaUrl('posts')}}" alt=""
                                class="object-fill !h-[50%] img-fluid"></a>
                    </figure>
                ...
                @empty
                @if($search=='')
                <p class="text-lg text-center text-red-500">Il n'y a encore rien dans cette catégorie.</p>
                @else
                <p class="text-lg text-center text-red-500">Pas de resultat pour <strong>{{$search}}</strong>.</p>
                @endif
            ...
            <div class="px-4 mx-2 lg:mx-0 lg:w-3/12">
                ...
                <div class="mt-4 widget tags">
                    ...
                    <div class="flex flex-wrap">
                        @foreach ($tags as $tag)
                        <a class="{{$tag_search ===$tag->tag ?'!bg-gray-400 !text-white':''}}" wire:click='addTag("{{$tag->tag}}",{{$tag->id}})'>{{$tag->tag}}</a>
                        @endforeach
